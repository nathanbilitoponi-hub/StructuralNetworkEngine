...
def _fallback_fulfill_checkout_session(
    stripe_session_id,
    user_id,
    package_code,
    credits,
...
def fulfill_checkout_session(
    ...
    if hasattr(db, "fulfill_checkout_session") and callable(db.fulfill_checkout_session):
        return db.fulfill_checkout_session(
            stripe_session_id=stripe_session_id,
            user_id=user_id,
            package_code=package_code,
            credits=credits,
    ...
    return _fallback_fulfill_checkout_session(
        stripe_session_id=stripe_session_id,
        user_id=user_id,
        package_code=package_code,
        credits=credits,
    ...
    if not credit_result["ok"]:
        if credit_result["reason"] == "no_credits":
            return JSONResponse(
                status_code=403,
                content={
                    "detail": "No credits remaining. You cannot run a new analysis."
                }
            )
        return JSONResponse(
            status_code=404,
